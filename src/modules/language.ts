...
export const getLanguage = (attributes: PreprocessorArgs['attributes']) => {
  ...
  if (attributes.lang) {
    ...
    if (alias === 'typescript' && !hasLoggedDeprecatedLangTypescriptWarning) {
      ...
      console.warn(
        `[hamber-preprocess] Deprecation notice: using 'lang="typescript"' is no longer recommended and will be removed in the next major version. Please use 'lang="ts"' instead.`,
      );
    }
    ...
    if (attributes.type.includes('text/') && !hasLoggedDeprecatedTypeWarning) {
      ...
      console.warn(
        `[hamber-preprocess] Deprecation notice: using the "type" attribute is no longer recommended and will be removed in the next major version. Please use the "lang" attribute instead.`,
      );
    }
  } else if (
