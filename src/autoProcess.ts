...
export function hamberPreprocess(
  {
    aliases,
  ...
  const style: PreprocessorGroup['style'] = async ({
    ...
    if (hasPostcss) {
      ...
      console.warn(
        `[hamber-preprocess] 'global' attribute found, but 'postcss' is not installed. 'postcss' is used to walk through the CSS and transform any necessary selector.`,
      );
    }
