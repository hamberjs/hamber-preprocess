...
import { compile } from 'hamber/compiler';
...
import pkg from 'hamber/package.json';
...
function getCompilerOptions({
  ...
  if (
    ...
    throw new Error(
      `Hamber only supports es6+ syntax. Set your 'compilerOptions.target' to 'es6' or higher.`,
    );
  }
...
const transformer: Transformer<Options.Typescript> = async ({
  ...
  if (!canUseMixedImportsTranspiler && options.handleMixedImports) {
    throw new Error(
      'You need at least Hamber 3.39 to use the handleMixedImports option',
    );
  }
