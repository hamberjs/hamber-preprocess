...
const transformer: Transformer<Options.Postcss> = async ({
  ...
  if (fileConfig?.error != null) {
    console.error(
      `[hamber-preprocess] PostCSS configuration was not passed or is invalid. If you expect to load it from a file make sure to install "postcss-load-config" and try again.\n\n${fileConfig.error}`,
    );
  }
