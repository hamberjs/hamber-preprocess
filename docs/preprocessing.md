...
`hamber-preprocess` can be used in two ways: _auto-preprocessing_ and with _stand-alone_ processors.

The examples below are going to be using a hypothetical `rollup.config.js`, but `hamber-preprocess` can be used in multiple scenarios, see "[Usage](/docs/usage.md)".

## Preprocessor Modes

### Auto Preprocessing

In auto preprocessing mode, `hamber-preprocess` automatically uses the respective preprocessor for your code based on a tag's `src`, `lang` or `type` attribute. It also handles the `<template>` tag for markup, external files and global styling.
...
As `hamber-preprocess` is just a Hamber preprocessor like any other, it's also possible to use it alongside other preprocessors:
...
| --------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `markupTagName` | `"template"` | `string` that sets the name of the tag `hamber-preprocess` looks for markup in custom languages.<br><br>i.e `markup` makes it possible to write your markup between `<markup lang="..."></markup>` tag.                            |
...
| `sourceMap`     | `false`      | If `true`, `hamber-preprocess` generates sourcemap for every language that supports it.                                                                                                                                            |
...
It's also possible to create custom preprocessors, taking advantage of the automatic language detection of `hamber-preprocess`:
...
Now `hamber-preprocess` will use the `potatoLanguage` preprocessor whenever it finds a tag with `lang="potato"` or `src="./index.pot"`.

These methods receive the same arguments and act exactly like a [common hamber preprocessor](https://hamberjs.web.app/docs#hamber_preprocess), but the concept of `markup`/`style`/`script` is abstracted as they are executed whenever `hamber-preprocess` find the aforementioned attributes.

##### Overriding preprocessors

We've seen that we can easily create custom preprocessors within `hamber-preprocess`, but wait, there's more! Using the same mechanism, it's possible to override the default preprocessor for a language!
...
To integrate `esbuild` with `hamber-preprocess` we can override the default TypeScript preprocessor as follows:
...
In case you want to manually configure your preprocessing step while taking advantage of `hamber-preprocess` features, such as language detection and external file support, the following preprocessors are available: Pug, CoffeeScript, TypeScript, Less, SCSS, Sass, Stylus, PostCSS, Babel, `globalStyle`, and `replace`.
...
```js
import { scss, postcss } from 'hamber-preprocess';

hamber.preprocess(input, [
  scss(),
  postcss({
    plugins: [
...
The auto preprocessing mode is the recommended way of using `hamber-preprocess` as it's the least verbose and most straightforward way of supporting multiple language preprocessors.

`hamber-preprocess` was built in a way that the underlying transpilers and compilers are only required/imported if a portion of your component matches its language. With that said, the auto-preprocessing mode is roughly equivalent to using all the stand-alone preprocessors in the following order:
...
Besides the options of each preprocessor, `hamber-preprocess` also supports these custom options:
...
`Hamber` expects your JavaScript to be in at least ES6 format, so make sure to set your Babel configuration accordingly.
...
Note: `hamber-preprocess` automatically configures inclusion paths for your root directory, `node_modules` and for the current file's directory.
...
In auto-preprocessing mode, you can set `postcss: true` if `postcss-load-config` is installed and `hamber-preprocess` will look for a PostCSS config file in your project.
...
**Hamber Element directives:**

Syntax to use Hamber Element directives with Pug
...
| `implementation` | `undefined` | pass the module to use to compile sass, if unspecified, `hamber-preprocess` will first look for `node-sass` and then for Sass. |
...
| `handleMixedImports` | inferred    | optional `boolean` that defines the transpilation strategy. If set to `true`, you don't need to strictly separate types and values in imports. You need at least Hamber version 3.39 if you want to use this. `true` by default if you meet the minimum version requirement, else `false`. This option will be ignored if you set `preserveValueImports` to `true` in your `tsconfig.json`. |
...
- Since `v4`, `hamber-preprocess` doesn't type-check your component, its only concern is to transpile it into valid JavaScript for the compiler. If you want to have your components type-checked, you can use [hamber-check](https://github.com/hamberjs/language-tools/blob/master/packages/hamber-check/README.md).

- Using TypeScript inside a component's markup is currently **not** supported.

### `globalStyle`

The `globalStyle` preprocessor extends the functionalities of Hamber's `:global` pseudo-selector.
...
- Wrapping `@keyframes` inside `:global {}` blocks is not supported. Use the [`-global-` name prefix for global keyframes](https://hamberjs.web.app/docs#style).\_
...
import { replace } from 'hamber-preprocess'
